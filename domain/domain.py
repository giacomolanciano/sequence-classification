...
class Domain(object):
    ...
    def get_sequence_label_data_by_label(self, label_name, table_name="protein"):

        self.cursor.execute("SELECT  sequence, class_label FROM " + table_name + " WHERE class_label =  ?",
                            (label_name,))
        table = self.cursor.fetchall()
        my_table = []
        for row in table:
